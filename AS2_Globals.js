...
$(document).ready(function() {
  ...
  var item = $(".dl-map"),
      ...
      list = $('#units-list'),
      gallery = $('#au-gallery'),
      feature = $('#au-features ul'),
      entity = list.data("entity");

  initMap(locLat, locLng);
  getUnits(entity, list, gallery, feature);
...
function getUnits(entity, list, gallery, feature) {

  var settings = {
        "async": true,
        "crossDomain": true,
        "url": "https://khaotic-staging.com/american-self-storage/wss_api/locationdata.php?method=hello&format=json&entity=" + entity,
        "dataType": "json",
        "method": "GET"
      },
      locationInfo = $.ajax(settings);

  $.when(locationInfo).done(function(result){
    var json      = JSON.parse(result.data),
        name      = json.Location.Name,
        dates     = json.Location.ReservationDates,
        units     = json.Location.Units,
        img       = json.Location.FacilityFeatures,
        feat      = json.Location.LocationFeatures,
        filtered  = [];

    console.log(json);

    $.each(units, function(x, y){
      var id        = y.UnitId,
          size      = y.UnitSize.toLowerCase(),
          img       = y.UnitTypeImage,
          rate      = y.Monthly,
          price     = '$' + rate +'.00',
          insurance = y.InsuranceOptions,
          desc      = y.SizeDescriptionsField;

      /* <div class="ut-img">' + img + '</div>
       * <div class="ut-rate">' + rate + '</div>*/
      list.append('<li data-unitId="' + id + '"><div class="unit-table flexy"><div class="ut-size"><span>' + size + '</span></div><div class="ut-desc"><span>' + desc + '</span></div><div class="ut-price"><span>' + price + '</span></div><div class="ut-reserve"><button onclick="reserveUnit(this)">Reserve</button></div></div></li>')
    });

    $.each(img, function(x, y){
      var img = decodeURIComponent(y.ImageUrl);

      gallery.append('<div class="au-gallery_item"><img src="' + img + '" alt="' + name + '"></div>');
    });

    $.each(feat, function(x, y) {
      var feat = y.Description;

      feature.append('<li>' + feat + '</li>');
    });
